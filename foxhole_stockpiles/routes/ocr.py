...
async def __scan_image(image: UploadFile, request: Request):
    ...
    ocr = OCR()
    start = time.time()
    stockpile: Stockpile = await ocr.extract_stockpile_from_image(
        image=image, file_name=api_key[:10]
    )
    end = time.time()
    ...
    logger.info(
        f"{stockpile.type}:{stockpile.name} ({width}x{height}). Scanned image in {end - start:.2f}."
    )
